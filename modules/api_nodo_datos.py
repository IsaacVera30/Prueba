...
class DatabaseManager:
    ...
    def _save_measurement_sync(self, data):
        """Guardar medición sincrónicamente - CORREGIDO para campos correctos"""
        try:
            with self._get_connection() as connection:
                cursor = connection.cursor()
                
                # Query corregido para campos correctos
                query = """
                ...
                values = (
                    ...
                    data.get('sys', 0),                   # sys (campo correcto)
                    data.get('dia', 0),                   # dia (campo correcto)
                    data.get('nivel', 'Sin datos'),       # nivel (campo correcto)
                    data.get('hr_ml', 0),                 # hr_ml
                    data.get('spo2_ml', 0),               # spo2_ml
                ...
                self.logger.debug(f"Medición guardada: Paciente {data.get('id_paciente')}, SYS: {data.get('sys')}, DIA: {data.get('dia')}, Nivel: {data.get('nivel')}")
    ...
    def get_latest_measurements(self, limit=20, patient_id=None):
        """Obtener últimas mediciones - CORREGIDO para campos correctos"""
        if not self.is_connected():
            return []
        ...
        try:
            with self._get_connection() as connection:
                ...
                if patient_id:
                    query = """
                    SELECT id, id_paciente, sys, dia, nivel, 
                           COALESCE(hr_ml, 0) as hr_ml, 
                           COALESCE(spo2_ml, 0) as spo2_ml, 
                           timestamp_medicion
                    FROM mediciones 
                    WHERE id_paciente = %s
                    ORDER BY id DESC 
    ...
    def create_tables_if_not_exist(self):
        """Crear tablas necesarias si no existen - CORREGIDO para estructura correcta"""
        if not self.is_connected():
            return False
        ...
        try:
            with self._get_connection() as connection:
                ...
                # Tabla mediciones - ESTRUCTURA CORREGIDA
                mediciones_table = """
