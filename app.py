...
ultima_estimacion = {
    ...
    "raw_ir": "---",  # Añadido para depuración en UI
    "raw_red": "---" # Añadido para depuración en UI
}
...
# Definición global de KEY_FILE_LOCATION_FALLBACK
KEY_FILE_LOCATION_FALLBACK = 'service_account.json' 
SCOPES = ['https://www.googleapis.com/auth/drive.file'] 
FOLDER_ID = os.environ.get('GOOGLE_DRIVE_FOLDER_ID', "ESTE_ES_UN_PLACEHOLDER_REEMPLAZAR_EN_RENDER") 
CSV_FILENAME = "registro_sensor_entrenamiento.csv"

if not FOLDER_ID or FOLDER_ID == "ESTE_ES_UN_PLACEHOLDER_REEMPLAZAR_EN_RENDER":
    print("⚠️ Advertencia: GOOGLE_DRIVE_FOLDER_ID no está configurado correctamente en las variables de entorno de Render.")
...
# --- FUNCIÓN MODIFICADA PARA GUARDAR SOLO id_paciente, sys, dia, nivel EN RAILWAY ---
def guardar_medicion_mysql(id_paciente_recibido, valor_sys_estimada, valor_dia_estimada, valor_nivel_calculado):
    ...
    cursor = conn.cursor()
    # Query SQL ajustada a las columnas de tu tabla en Railway
    query = """
    INSERT INTO mediciones (id_paciente, sys, dia, nivel) 
    VALUES (%s, %s, %s, %s)
    """
    datos_a_insertar = (id_paciente_recibido, valor_sys_estimada, valor_dia_estimada, valor_nivel_calculado)
    
    try:
        print(f"guardar_medicion_mysql: Intentando insertar: {datos_a_insertar}")
        cursor.execute(query, datos_a_insertar)
        conn.commit()
        print(f"Datos guardados en MySQL: IDP={id_paciente_recibido}, SYS={valor_sys_estimada}, DIA={valor_dia_estimada}, Nivel={valor_nivel_calculado}")
        return True
    except mysql.connector.Error as err:
        ...
        if conn.is_connected(): 
            cursor.close()
            conn.close()
            print("Conexión a MySQL cerrada (guardar_medicion_mysql).")
# --- FIN FUNCIÓN MODIFICADA ---

def get_google_drive_service():
    global KEY_FILE_LOCATION_FALLBACK
...
def subir_archivo_a_drive(file_path, file_name_on_drive):
    if not FOLDER_ID or FOLDER_ID == "ESTE_ES_UN_PLACEHOLDER_REEMPLAZAR_EN_RENDER":
        print(f"Error Drive: FOLDER_ID no configurado. Valor actual: {FOLDER_ID}")
        return False
    ...
    try:
        ...
        if existing_files: 
            ...
            print(f"Archivo '{file_name_on_drive}' actualizado en Drive (Carpeta ID: {FOLDER_ID}).")
        else: 
            file = service.files().create(body=file_metadata, media_body=media, fields='id').execute()
            print(f"Archivo '{file_name_on_drive}' subido a Drive (Carpeta ID: {FOLDER_ID}).")
        return True
    except Exception as e: 
        print(f"❌ Error al subir/actualizar archivo en Google Drive: {e}")
        return False
...
def api_procesar_presion():
    ...
    try:
        ...
        except ValueError: return jsonify({"error": "Datos deben ser numéricos enteros"}), 400

        # Lógica de SpO2 de tu código origen
        ...
        if len(ventana_ir) == MUESTRAS:
            if ir_val < 10000 or red_val < 10000:
                ...
                try:
                    ...
                    else: print(f"DEBUG: SpO2 - DC/AC inválido.")
        ...
        # --- MODIFICACIÓN: Incluir raw_ir y raw_red en la respuesta ---
        current_timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ultima_estimacion = {
            "sys": f"{sys_estimada:.2f}" if sys_estimada != -1 else "---",
            "dia": f"{dia_estimada:.2f}" if dia_estimada != -1 else "---",
            "spo2": f"{spo2_estimada:.1f}" if spo2_estimada > 0 else "---",
            "hr": str(hr), "nivel": nivel_presion,
            "timestamp": current_timestamp, "modo_autorizado": autorizado,
            "raw_ir": ir_val,
            "raw_red": red_val
        }
        # --- FIN MODIFICACIÓN ---
        ...
        if autorizado:
            try:
                with open(CSV_FILENAME, mode='a', newline='') as file_csv:
                    file_csv.write(f"{hr},{spo2_estimada:.1f},{ir_val},{red_val},{sys_estimada:.2f},{dia_estimada:.2f},{current_timestamp}\n")
                print(f"Datos guardados en {CSV_FILENAME} para entrenamiento.")
                if FOLDER_ID and FOLDER_ID != "ESTE_ES_UN_PLACEHOLDER_REEMPLAZAR_EN_RENDER":
                     subir_archivo_a_drive(CSV_FILENAME, CSV_FILENAME)
            except Exception as e_csv: print(f"Error al guardar o subir CSV: {e_csv}")

        print(f"DEBUG: Antes de guardar en DB - ir_val={ir_val}, red_val={red_val}, sys_estimada={sys_estimada}")
        if ir_val > 20000 and red_val > 15000 and sys_estimada != -1: # Umbrales de tu código origen
            guardar_medicion_mysql(id_paciente_in, sys_estimada, dia_estimada, nivel_presion) # Solo 4 campos
        else: print("DEBUG: Condición para guardar en DB no cumplida.")

        return jsonify({
            "sys": round(sys_estimada, 2) if sys_estimada != -1 else -1, 
            "dia": round(dia_estimada, 2) if dia_estimada != -1 else -1, 
...
def get_ultimas_mediciones_db():
    ...
    # Query para obtener las columnas que existen en tu tabla Railway
...
if __name__ == "__main__":
    ...
    if not FOLDER_ID or FOLDER_ID == "ESTE_ES_UN_PLACEHOLDER_REEMPLAZAR_EN_RENDER": 
        print("ADVERTENCIA: GOOGLE_DRIVE_FOLDER_ID no configurado correctamente.")
